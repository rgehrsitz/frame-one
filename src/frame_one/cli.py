...
def main() -> None:
    ...
    if args.claude_status_file:
        claude_state = claude_provider_state(args.claude_status_file)
        if claude_state["state"] != "ok":
            parser.error("Claude Code status snapshot unavailable; existing display was left untouched")
